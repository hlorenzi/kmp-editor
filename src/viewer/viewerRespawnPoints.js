...
class ViewerRespawnPoints
{
	...
	refreshPanels()
	{
		...
		panel.addSelectionNumericInput(selectionGroup,   "Size",        1,    1000, selectedPoints.map(p =>  p.size),         1.0, 1.0, enabled, multiedit, (x, i) => { this.window.setNotSaved(); selectedPoints[i].size = x })
	}
	...
	toggleAllSelection()
	{
		let hasSelection = (this.data.respawnPoints.nodes.find(p => p.selected) != null)
	...
	onMouseDown(ev, x, y, cameraPos, ray, hit, distToHit, mouse3DPos)
	{
		...
		if (hoveringOverElem != null)
		{
			if (ev.altKey)
			{
				let newPoint = this.data.respawnPoints.addNode()
