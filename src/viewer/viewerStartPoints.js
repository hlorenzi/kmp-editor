...
class ViewerStartPoints
{
	...
	toggleAllSelection()
	{
		let hasSelection = (this.data.startPoints.nodes.find(p => p.selected) != null)
	...
	deleteSelectedPoints()
	{
		...
		for (let point of pointsToDelete)
			this.data.startPoints.removeNode(point)
	...
	onMouseDown(ev, x, y, cameraPos, ray, hit, distToHit, mouse3DPos)
	{
		...
		if (hoveringOverElem != null)
		{
			if (ev.altKey)
			{
				let newPoint = this.data.startPoints.addNode()
